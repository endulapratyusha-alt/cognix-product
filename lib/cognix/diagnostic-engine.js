...
const FRAME_TERMS = {
  ...
  revenue: ["pipeline", "revenue", "conversion", "win rate", "growth"],
...
const AUDIENCE_TERMS = {
  ...
  revops: ["revops", "operations", "gtm leadership"],
...
const COMMERCIAL_TERMS = ["pipeline", "revenue", "conversion", "win rate", "retention", "expansion", "pricing", "budget", "paid", "roi", "sales cycle"];
...
function detectCrossSignalFindings(bundle) {
  ...
  if (map.frame_conflicts.length) {
    risks.push(risk(
      "Narrative drift",
      "High",
      "Core materials are leading with different value frames, so buyers and reps may not retain one coherent reason to care.",
...
function detectStrengths(bundle) {
  ...
  return strengths.length ? strengths : ["The team has begun centralizing GTM signals, which is the foundation for coherence analysis."];
}

function buildRecommendedPriorities(risks) {
  const recommendations = risks.slice(0, 5).map((riskItem) => {
    const map = {
      "Narrative drift": ["PMM", "Choose one controlling narrative and rewrite website, sales, and launch materials around the same buyer tension, outcome, and proof.", "Creates one field-ready story instead of parallel narratives."],
      ...
      "Commercial wedge weakness": ["Leadership", "Anchor the diagnostic type to a commercial wedge: pipeline risk, conversion lift, sales consistency, launch success, or competitive win rate.", "Makes the product easier to buy, fund, and prioritize."],
      "Weak conversion flow": ["Marketing", "Add a clear next step to website, launch, and sales materials so the GTM narrative turns into pipeline motion.", "Reduces interest-to-opportunity leakage."],
      ...
      "Execution alignment issue": ["Enablement", "Translate the strategy into talk tracks, objections, handoffs, launch owners, and rep usage moments.", "Turns strategy into field behavior."],
      "Strategic contradiction": ["PMM", "Resolve contradictory claims by creating a message hierarchy: primary promise, secondary proof, and channel-specific adaptations.", "Prevents buyers from hearing competing explanations of the product."]
    };
    const [owner, action, expected_impact] = map[riskItem.risk] || ["PMM", `Address ${riskItem.risk.toLowerCase()} using the cited evidence.`, "Improves GTM coherence."];
    return { action, owner, priority: riskItem.severity, expected_impact };
  });
  return recommendations.length ? recommendations : [{
    action: "Add more signals across website, positioning, sales, launch, enablement, customer, and competitor surfaces, then rerun the diagnostic.",
    owner: "PMM",
    priority: "Medium",
...
function buildDiagnosis(risks, bundle) {
  return `Cognix compared ${bundle.metadata.signal_count} signals across ${bundle.metadata.signal_types.join(", ")} and found the primary GTM coherence risk in ${risks[0].risk.toLowerCase()}. This is a cross-signal diagnosis, not an asset summary.`;
}
...
function buildExecutiveSummary(project, risks, scores, options) {
  ...
  return `${prefix}${project.company_name || "This company"} has an overall GTM coherence score of ${scores.overall}. The strongest issue is ${risks[0].risk.toLowerCase()}, supported by evidence across submitted GTM signals. Fixing this first should improve buyer clarity, field consistency, and conversion focus before the team creates more assets.`;
}
...
function lowConfidenceRisk(signal) {
  return risk("GTM coherence gap", "Low", "There are not enough varied GTM signals to make a high-confidence cross-signal diagnosis.", [`${signal.title} is the only evidence source currently available.`]);
}
